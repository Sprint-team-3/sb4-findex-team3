...
public class BasicIndexDataService implements IndexDataService {
  ...
  public Page<IndexDataDto> searchByIndexAndDate(IndexDataDateRequest request, Pageable pageable) {
    IndexInfo indexInfo =
        infoRepository
            .findById(request.indexInfo()) // infoRepository에서 일단 id를 찾고
            .orElseThrow(() -> new EntityNotFoundException("IndexInfo not found!"));
    // indexInfo와 request를 통해 받은 값들을 입력해서 찾는다
    Page<IndexData> searchData =
        dataRepository.findByIndexInfoAndBaseDateBetween(
            indexInfo, request.startDate(), request.endDate(), pageable);
  ...
  /**
   * 지수 데이터 Export 1. 지수 데이터 전체를 Export 하는거니까 List<IndexDataDto> 형태로 전부 찾는다 2. OutputStream 생성 3.
   * CSVWriter를 하나 만든다 4. CSVWriter에 Header - Body 순으로 작성한다 CSVWriter.
   */
  @Override
  public byte[] downloadIndexData() {
    ...
    try {
      ...
      CSVWriter csvWriter =
          new CSVWriter(new OutputStreamWriter(outputStream, "UTF-8")); // 직렬화, 역직렬화를 할땐 예외 필수
